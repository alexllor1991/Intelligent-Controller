...
public class Arguments {
    ...
    public static Options getOptions() {
        ...
        options.addOption(Option.builder("g").longOpt("graphics").argName("GRAPHICS").desc("Training with graphics").build());
        options.addOption(Option.builder("b").longOpt("batch-size").argName("BATCH-SIZE").desc("Batch size of the training data").build());
        options.addOption(Option.builder("p").longOpt("pre-trained").argName("PRE-TRAINED").desc("Use pre-trained weights").build());
        options.addOption(Option.builder("t").longOpt("testing").argName("TESTING").desc("Test the trained model").build());
        return options;
    }
